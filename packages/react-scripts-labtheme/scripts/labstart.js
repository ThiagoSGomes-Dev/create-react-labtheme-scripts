...
const fs = require('fs-extra'); // labtheme - touched
const chalk = require('react-dev-utils/chalk');
const webpack = require('webpack');
//const WebpackDevServer = require('webpack-dev-server'); // labtheme - remarked out
...
const paths = require('../config/paths-labtheme'); // labtheme - touched
const configFactory = require('../config/webpack.config.labtheme'); // labtheme - touched
//const createDevServerConfig = require('../config/webpackDevServer.config'); // labtheme - remarked out
...
// labtheme - added section - start
const config = configFactory('development');
const appPackage = require(paths.appPackageJson);
const labThemeUserConfig = require('create-react-labtheme-utils/getUserConfig')(
  paths,
  process.env.NODE_ENV
);
const labThemePostInstallerInfo = require('create-react-labtheme-utils/postInstallerInfo');
const labThemeFileFunctions = require('create-react-labtheme-utils/fileFunctions');

const copyPublicFolder = labThemeFileFunctions.copyPublicFolder;
const copyToThemeFolder = labThemeFileFunctions.copyToThemeFolder;
const deleteDeployFolder = labThemeFileFunctions.deleteDeployFolder;
const setupCopyToThemeFolder = labThemeFileFunctions.setupCopyToThemeFolder;
const writeDoNotEditFile = labThemeFileFunctions.writeDoNotEditFile;

const _labThemeServer =
  labThemeUserConfig &&
  labThemeUserConfig.labThemeServer &&
  labThemeUserConfig.labThemeServer.enable === true
    ? require('create-react-labtheme-utils/labThemeServer')
    : null;
// labtheme - added section - end
...
function startWatch() {
  ...
  const injectlabThemeClient = function (labThemeServer) {
    if (!labThemeUserConfig) {
      return;
    }

    let clientConfig = labThemeUserConfig.injectlabThemeClient;
    if (clientConfig) {
      clientConfig.mode =
    ...
    let toInject = labThemeServer.getClientInjectString(
      clientConfig.mode,
      clientConfig.token
    );
  ...
  return new Promise((resolve, reject) => {
    ...
    if (
      doLaunchBrowser &&
      isInteractive &&
      labThemePostInstallerInfo.postInstallerExists(paths)
    ) {
      clearConsole();
    ...
    // doLaunchBrowser is only true on startup
    if (doLaunchBrowser && _labThemeServer) {
      _labThemeServer.startServer(paths);
    }
    ...
    const devSocket = {
      warnings: (warnings) => {
        if (_labThemeServer) {
          _labThemeServer.update(warnings, 'warnings');
        }
      },
      errors: (errors) => {
        if (_labThemeServer) {
          _labThemeServer.update(errors, 'errors');
        }
      },
    };
    ...
    const watcher = compiler.watch(watchOptions, (err, stats) => {
      ...
      injectLabThemeClient(_labThemeServer);
      copyToThemeFolder(paths);
      writeDoNotEditFile(paths);

      if (_labThemeServer) {
        _labThemeServer.update(stats);
      }
